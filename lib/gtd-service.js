...
gtdService = {
  ...
  parseLine: function (line, dateFormat) {
    var task, words, i, j, word, completed, completionDate, priority, date, meta, metaKey;
    task = {};
    words = line.split(' ');
    j = 0;
    for (i = 0; i < words.length; i++) {
      ...
      meta = word.match(/(\S+):(\S+)/);
      if (meta) {
        metaKey = meta[1].toUpperCase();
        if (metaKey.toUpperCase() === 'DUE') {
          task.dueDate = gtdService.parseDate(meta[2], dateFormat);
          continue;
        } else if (metaKey === 'DUEFREQ') {
          task.dueFreq = meta[2];
          continue;
        }
        /*} else if (metaKey === meta[1]) {
         task.meta = task.meta || {};
  ...
  saveFileData: function (todoFile, doneFile) {
    ...
    tasks.forEach(function (task) {
      var line, key, searchNextDueDate;
      searchNextDueDate = task.dueFreq && task.dueDate && task.completed;
      while (searchNextDueDate && !moment().isBefore(task.dueDate)) {
        switch (task.dueFreq) {
          case 'weekly':
            task.dueDate = moment(task.dueDate).add('weeks', 1).toDate();
            break;
          case 'monthly':
            task.dueDate = moment(task.dueDate).add('months', 1).toDate();
            break;
          case 'yearly':
            task.dueDate = moment(task.dueDate).add('years', 1).toDate();
            break;
          default:
            searchNextDueDate = false;
            break;
        }
      }
      if (searchNextDueDate) {
  ...
  getTasks: function (search) {
    ...
    if (search) {
      ...
      result = tasks.filter(function (task) {
        if (task.description && task.description.toLowerCase().indexOf(search) !== -1) {
          return true;
        } else {
          return false;
        }
